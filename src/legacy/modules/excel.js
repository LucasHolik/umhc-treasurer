...
function parseAndCleanData(rows) {
  ...
  for (let i = headerIndex + 1; i < rows.length; i++) {
    ...
    if (
      ...
      rowText.includes("Pending Transactions (Submitted, Waiting Approval)")
    ) {
      break;
    }
    ...
    if (date) {
      ...
      if (
        row[documentCol] &&
        !currentTransaction.document.includes(String(row[documentCol]))
      ) {
        currentTransaction.document += "\n" + row[documentCol];
      }
      if (
        row[descriptionCol] &&
        !currentTransaction.description.includes(String(row[descriptionCol]))
      ) {

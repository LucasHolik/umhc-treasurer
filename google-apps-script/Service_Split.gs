var Service_Split = {
  ...
  processSplit: function (e) {
    ...
    try {
      ...
      // Dynamic indices from CONFIG
      const descIndex = CONFIG.HEADERS.indexOf("Description");
      const tripIndex = CONFIG.HEADERS.indexOf("Trip/Event");
      const catIndex = CONFIG.HEADERS.indexOf("Category");
      ...
      splits.forEach((split) => {
        ...
        if (descIndex !== -1) childRow[descIndex] = split.Description;

        // Update Tags if provided (otherwise they inherit from originalRowValues)
        if (split.TripEvent !== undefined && tripIndex !== -1)
          childRow[tripIndex] = split.TripEvent;
        if (split.Category !== undefined && catIndex !== -1)
          childRow[catIndex] = split.Category;
        ...
        if (isIncome) {
          if (incomeIndex !== -1) childRow[incomeIndex] = split.Amount;
          if (expenseIndex !== -1) childRow[expenseIndex] = "";
        } else {
          if (incomeIndex !== -1) childRow[incomeIndex] = "";
          if (expenseIndex !== -1) childRow[expenseIndex] = split.Amount;
        }
  ...
  removeTagFromSplits: function (type, value) {
    ...
    const headers = splitSheet
      ...
      .getValues()[0];
    const colIndex = headers.indexOf(type) + 1; // 1-based column index

    if (colIndex <= 0) {
      return { success: false, message: "Column not found: " + type };
    }
  ...
  updateSplitRowTag: function (rowId, tripEvent, category) {
    ...
    const headers = splitSheet
      .getRange(1, 1, 1, splitSheet.getLastColumn())
      .getValues()[0];
    const tripIndex = headers.indexOf("Trip/Event") + 1;
    const catIndex = headers.indexOf("Category") + 1;

    if (tripIndex > 0)
      splitSheet.getRange(rowIndex, tripIndex).setValue(tripEvent);
    if (catIndex > 0)
      splitSheet.getRange(rowIndex, catIndex).setValue(category);
  ...
  editSplit: function (e) {
    ...
    if (financeRowIndex === -1) {
      return {
        success: false,
        message:
          "Original transaction not found in Finance Sheet for ID: " + groupId,
      };
    }
  ...
  clearSplitSheet: function () {
    try {
      const splitSheet = _getSplitSheet();
      const lastRow = splitSheet.getLastRow();
      if (lastRow > 1) {
        splitSheet.deleteRows(2, lastRow - 1);
      }
      return { success: true, message: "Split sheet cleared." };
    } catch (error) {
      console.error("Error clearing split sheet:", error);
      return {
        success: false,
        message: "Error clearing split sheet: " + error.message,
      };
    }
  },

  restoreSplitHistory: function (e) {
    try {
      const splits = JSON.parse(e.parameter.splits || "[]");
      if (splits.length === 0)
        return { success: true, message: "No splits to restore." };
      ...
      const rows = splits.map((split) => {
        return headers.map((header) => {
          return split[header] !== undefined ? split[header] : "";
        });
      });
      ...
      return {
        success: true,
        message: "Restored " + splits.length + " split records.",
      };
    } catch (error) {
      console.error("Error restoring splits:", error);
      return {
        success: false,
        message: "Error restoring splits: " + error.message,
      };
    }
  },
};

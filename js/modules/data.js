...
function findUniqueRecords(newData, existingData) {
  ...
  for (const existingRow of existingData) {
    ...
    // For income and expense, normalize number values to handle formatting differences like commas
    const incomeStr = normalizeValue(
      formatNumberForComparison(existingRow.Income)
    );
    const expenseStr = normalizeValue(
      formatNumberForComparison(existingRow.Expense)
    );
    ...
    if (existingKeys.size < 2) {
      ...
      console.log("Existing sheet key components:", {
        ...
        normalizedDescription: descriptionStr,
        document: existingRow.Document,
        normalizedDocument: documentStr,
        income: existingRow.Income,
        normalizedIncome: incomeStr,
        expense: existingRow.Expense,
        normalizedExpense: expenseStr,
      });
  ...
  for (const row of newData) {
    ...
    // For cashIn and cashOut, normalize number values to handle formatting differences like commas
    const incomeStr = normalizeValue(formatNumberForComparison(row.cashIn));
    const expenseStr = normalizeValue(formatNumberForComparison(row.cashOut));

    // Join components with the same delimiter
    const key = `${dateStr}|${descriptionStr}|${documentStr}|${incomeStr}|${expenseStr}`;

    // Debug: log an example key from new data
    if (duplicateCount + uniqueRecords.length < 2) {
      // Only log first few for debugging
  ...
  console.log(
    `Found ${uniqueRecords.length} unique records and ${duplicateCount} duplicates`
  );
  return uniqueRecords;
}

// Function to format numbers for comparison (removes commas, ensures consistent format)
function formatNumberForComparison(value) {
  ...
  // If it's a valid number after removing commas, convert to standard number format
  const numValue = Number(strValue);
  if (!isNaN(numValue)) {
    // Convert to a consistent string format for number comparison
    return numValue.toString();
  }

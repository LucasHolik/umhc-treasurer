...
function findUniqueRecords(newData, existingData) {
  ...
  for (const existingRow of existingData) {
    ...
    const dateStr = normalizeValue(existingRow.Date);
  ...
  for (const row of newData) {
    // Create normalized key components for the new row
    const dateStr = normalizeValue(row.date);
...
function normalizeValue(value) {
  ...
  // Handle dates by converting to consistent YYYY-MM-DD format
  ...
  // Check if it looks like a date string (from Google Sheets or Excel)
